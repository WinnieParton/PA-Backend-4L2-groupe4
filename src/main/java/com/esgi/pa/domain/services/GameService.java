...
public class GameService {
  ...
  public Game createGame(
    ...
    if (gameAdapter.findByName(name)) throw new TechnicalFoundException(
      "A game using this name already exist : " + name
    );
  ...
  public String runEngine(Lobby lobby, String jsonData) {
    ...
    if (
      ...
      !output.contains("Erreur lors de l'exécution")
    ) moveService.saveGameState(lobby, output, ActionEnum.OUTPUT);
    if (!output.contains("Erreur lors de l'exécution")) return output;
    return "";
  }
  ...
  public String runScriptPython(Lobby lobby, String jsonData) {
    try {
      ...
      if (
        isNewInstance || process == null || writer == null || reader == null
      ) {
        String pythonCommand = getPythonCommand();
        if (pythonCommand == null) {
          throw new IOException(
            "Python or Python3 not found in the environment"
          );
        }
        createNewInstance(
          pythonCommand,
          UPLOAD_DIR + lobby.getGame().getGameFiles()
        );
      }
   ...
   */
  private void createNewInstance(String type, String fileName)
    throws IOException {
  ...
  private String getPythonCommand() {
    ...
    try {
      ...
      if (
        line != null &&
        (line.startsWith("Python 2") || line.startsWith("Python 3"))
      ) {
        return "python";
      }
    } catch (IOException e) {
